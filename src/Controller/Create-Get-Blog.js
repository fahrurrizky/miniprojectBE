...
const createBlog = async (req, res) => {
  ...
  if (!user || !user.isVerified) {
    return res.status(403).json({ message: "Account not verified, please check your email for link verification! / Registration First" });
  }
  ...
  try {
    await db.sequelize.transaction(async (t) => {
      ...
      return res.status(200).json({ message: "Blog created successfully", data: result });
    });
  } catch (err) {
...
const getBlogbyQuery = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "successfully get data", page, limit, data: blog });
  } catch {
    res.status(500).json({ message: "data failed to fetch" });
  }
};
